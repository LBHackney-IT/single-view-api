...
namespace SingleViewApi.Tests.V1;
...
public class HttpHeadersExtensionsTests
{
    ...
    private readonly Mock<IHeaderDictionary> _mockHeaders = new();

    private delegate void SubmitCallback(string x, out StringValues y);

    [Test]
    public void GetHeaderValueThrowsNullHeaders()
    {
        var func = () => HttpHeadersExtensions.GetHeaderValue(null, Key);
    ...
    public void GetHeaderValueFoundManyKeyValuesReturnsFirst()
    {
        StringValues outVal;
        _mockHeaders.Setup(x => x.TryGetValue(Key, out outVal))
            .Callback(new SubmitCallback((string x, out StringValues y) =>
                y = new StringValues(new[] { Value, "val 2", "val 3" })))
            .Returns(true);
        _mockHeaders.Object.GetHeaderValue(Key).Should().Be(Value);
    }
}
