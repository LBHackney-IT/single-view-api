...
public class GetCustomerByIdUseCaseTests : LogCallAspectFixture
{
    ...
    public void GetsACustomerFromPersonApiJigsawAndAcademyCouncilTax()
    {
        ...
        _mockCustomerGateway.Setup(x => x.Find(id)).Returns(new SavedCustomer
        {
            ...
            {
                new() { CustomerId = id, DataSourceId = 2, SourceId = mockJigsawId },
                new() { CustomerId = id, DataSourceId = 1, SourceId = mockPersonApi },
                new() { CustomerId = id, DataSourceId = 3, SourceId = mockCouncilTaxId },
                new() { CustomerId = id, DataSourceId = 4, SourceId = mockHousingBenefitsId }
            }
        });

        _mockSharedPlanGateway.Setup(x => x.GetSharedPlans(It.IsAny<GetSharedPlanRequest>()))
            .ReturnsAsync(new SharedPlanResponseObject { PlanIds = new List<string> { "101010", "202020", "303030" } });

        var fakeContactDetails = new List<CustomerContactDetails> { _fixture.Create<CustomerContactDetails>() };
        ...
        {
            Id = mockPersonApi,
            Title = Title.Miss.GetDisplayName(),
            FirstName = mockFirstName,
            ...
            PreferredTitle = Title.Miss.GetDisplayName(),
            PlaceOfBirth = null,
            PreferredFirstName = mockFirstName,
            PreferredMiddleName = null,
            PersonTypes = new List<string> { PersonType.Tenant.ToDescription() },
            CautionaryAlerts = fakeCautionaryAlerts,
            EqualityInformation = fakeEqualityInformation
        };

        _mockGetPersonApiByIdUseCase.Setup(x => x.Execute(mockPersonApi, userToken)).ReturnsAsync(
            new CustomerResponseObject
            {
                Customer = peronsApiCustomer,
                SystemIds = new List<SystemId> { new() { Id = mockPersonApi, SystemName = mockPersonApiName } }
            });

        var fakeJigsawContactDetails = new List<CustomerContactDetails> { _fixture.Create<CustomerContactDetails>() };
        ...
        _mockGetJigsawCustomerByIdUseCase.Setup(x => x.Execute(mockJigsawId, redisId, userToken)).ReturnsAsync(
            new CustomerResponseObject
            {
                Customer = jigsawApiCustomer,
                SystemIds = new List<SystemId> { new() { Id = mockJigsawId, SystemName = mockJigsawName } }
            });
        ...
        _mockGetCouncilTaxAccountByAccountRefUseCase.Setup(x => x.Execute(mockCouncilTaxId, userToken))
            .ReturnsAsync(new CustomerResponseObject
            {
                ...
                SystemIds = new List<SystemId> { new() { Id = mockCouncilTaxId, SystemName = mockCouncilTaxName } }
            });
        ...
        result.Customer.AllContactDetails[0].DataSourceName.Should()
            .BeEquivalentTo(fakeJigsawContactDetails[0].DataSourceName);
        result.Customer.AllContactDetails[0].IsActive.Should().Be(fakeJigsawContactDetails[0].IsActive);
        result.Customer.AllContactDetails[0].SourceServiceArea.Should()
            .BeEquivalentTo(fakeJigsawContactDetails[0].SourceServiceArea);
        result.Customer.AllContactDetails[0].ContactType.Should()
            .BeEquivalentTo(fakeJigsawContactDetails[0].ContactType);
        ...
        result.Customer.AllContactDetails[0].Description.Should()
            .BeEquivalentTo(fakeJigsawContactDetails[0].Description);
        result.Customer.AllContactDetails[0].AddressExtended.Should()
            .BeEquivalentTo(fakeJigsawContactDetails[0].AddressExtended);

        result.Customer.AllContactDetails[1].DataSourceName.Should()
            .BeEquivalentTo(fakeContactDetails[0].DataSourceName);
        result.Customer.AllContactDetails[1].IsActive.Should().Be(fakeContactDetails[0].IsActive);
        result.Customer.AllContactDetails[1].SourceServiceArea.Should()
            .BeEquivalentTo(fakeContactDetails[0].SourceServiceArea);
        ...
        result.Customer.AllContactDetails[1].AddressExtended.Should()
            .BeEquivalentTo(fakeContactDetails[0].AddressExtended);
        ...
        result.Customer.CautionaryAlerts.Count.Should()
            .Be(fakeCautionaryAlerts.Count + fakeJigsawCautionaryAlerts.Count);
