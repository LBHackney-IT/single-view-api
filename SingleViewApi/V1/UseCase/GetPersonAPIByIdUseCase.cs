...
public class GetPersonApiByIdUseCase : IGetPersonApiByIdUseCase
{
    ...
    public async Task<CustomerResponseObject> Execute(string personId, string userToken)
    {
        ...
        var response = new CustomerResponseObject { SystemIds = new List<SystemId> { personApiId } };
        ...
        else
        {
            ...
            foreach (var personType in person.PersonTypes) personTypes.Add(personType.ToDescription());

            var knownAddresses = person.Tenures.Map(t =>
            {
