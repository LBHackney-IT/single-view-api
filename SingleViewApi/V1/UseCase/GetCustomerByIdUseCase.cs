...
public class GetCustomerByIdUseCase : IGetCustomerByIdUseCase
{
    ...
    public MergedCustomerResponseObject Execute(Guid customerId, string userToken, string redisId)
    {
        ...
        var foundRecords = new List<CustomerResponseObject>();
        ...
        foreach (var customerDataSource in customer.DataSources)
        {
            ...
            switch (customerDataSource.DataSourceId)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    res = _getCouncilTaxAccountByAccountRefUseCase.Execute(customerDataSource.SourceId, userToken)
                        .Result;
                    foundRecords.Add(res);
                    break;
                case 4:
                    res = _getHousingBenefitsAccountByAccountRefUseCase.Execute(customerDataSource.SourceId, userToken)
                        .Result;
                    foundRecords.Add(res);
                    break;
            }
    ...
    private static MergedCustomerResponseObject MergeRecords(SavedCustomer customer,
        List<CustomerResponseObject> records, SharedPlanResponseObject sharedPlanIds)
    {
        ...
        foreach (var r in records)
        {
            ...
            if (r.Customer != null)
            {
                if (r.Customer.KnownAddresses != null) allKnownAddresses.AddRange(r.Customer.KnownAddresses);
                if (r.Customer.AllContactDetails != null) allContactDetails.AddRange(r.Customer.AllContactDetails);
                if (r.Customer.PersonTypes != null) allPersonType.AddRange(r.Customer.PersonTypes);

                if (r.Customer.CautionaryAlerts != null) allCautionaryAlerts.AddRange(r.Customer.CautionaryAlerts);
        ...
        return new MergedCustomerResponseObject { SystemIds = allSystemIds, Customer = mergedCustomer };
    }
}
